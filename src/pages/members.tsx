...
export default function Members() {
  ...
  const renderCell = React.useCallback((user: any, columnKey: any) => {
    ...
    switch (columnKey) {
        ...
        return (
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col'>
              ...
              <span className='text-xs text-default-400'>{user.data.nickname}</span>
            </div>
          </div>
        );
      ...
      case 'furnace':
        return (
          <img alt={cellValue} className='w-6 h-6 rounded-lg object-cover' src={user.data.stove_lv_content} />
        );
      default:
        return cellValue;
    }
    ...
    <section className='py-20 px-4 bg-dark-darker' id='members'>
      <div className='container mx-auto'>
        ...
        <Table aria-label='Example table with custom cells' className='w-full'>
          <TableHeader columns={columns} className='bg-dark-darker text-white'>
            {(column) => (
              <TableColumn
                key={column.uid}
